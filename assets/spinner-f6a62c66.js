        ...
        <ul class="exercises_list_item_bottom_list">
          <li class="exercises_list_item_bottom_list_item">
            <p class="exercises_list_item_bottom_list_item_text">
              ...
              </button>`}function $(t){return`<div class="modal-info" data-id="${t._id}">
    ...
    <div class="get-rating-container">
      ...
      <form class="form raiting-form">
        ...
        <div class="muscles-box-menu">
          ...
          `}).join("")}const h={quoteText:document.querySelector(".quote-text"),quoteAuthor:document.querySelector(".quote-author")};document.addEventListener("DOMContentLoaded",Re);async function Re(){const t=localStorage.getItem("quote");if(t){const e=new Date().toDateString(),{date:s,quote:a,author:o}=JSON.parse(t);e!==s?M():(h.quoteText.innerHTML=a,h.quoteAuthor.innerHTML=o)}else M()}async function M(){const t=await c.getQuotes(),{author:e,quote:s}=t,a={author:e,quote:s,date:new Date().toDateString()};localStorage.setItem("quote",JSON.stringify(a)),h.quoteText.innerHTML=s,h.quoteAuthor.innerHTML=e}function Pe(){const t=document.createElement("button");t.className="scroll-up-button hidden";const e=document.createElementNS("http://www.w3.org/2000/svg","svg");e.setAttribute("viewBox","0 0 32 32"),e.setAttribute("class","button-icon");const s=document.createElementNS("http://www.w3.org/2000/svg","use");s.setAttributeNS("http://www.w3.org/1999/xlink","href","/img/svg/sprite.svg#icon-arrow"),e.appendChild(s),t.appendChild(e),t.addEventListener("click",function(){window.scrollTo({top:0,behavior:"smooth"})}),document.addEventListener("scroll",function(){window.scrollY>50?t.classList.remove("hidden"):t.classList.add("hidden")}),document.body.appendChild(t)}Pe();function Fe(){const t=document.querySelector(".rating-container-js"),e=t.querySelectorAll(".star-js");let s=t.querySelector(".user-rating-js");e.forEach((i,l)=>{i.addEventListener("mouseover",()=>a(l)),i.addEventListener("mouseout",o),i.addEventListener("click",()=>n(l+1))});function a(i){o();for(let l=0;l<=i;l++)e[l].classList.add("hovered"),s.textContent=`${l+1}`}function o(){e.forEach(i=>{const l=t.dataset.rating;i.classList.remove("hovered"),s.textContent=`${l||0}`})}function n(i){t.setAttribute("data-rating",i),e.forEach((l,X)=>{X+1<=i?l.classList.add("selected"):l.classList.remove("selected")})}}const v=[];function Ue(t){v.push(t),localStorage.setItem("favorites",JSON.stringify(v))}const De=t=>{console.log(v),console.log(t),localStorage.setItem("favorites",JSON.stringify([...v.filter(e=>e._id!==t)]))},Y="/sportIsLife/assets/sprite-ffba578a.svg";console.log(Y);const Je=document.querySelector(".filter-list-js");let O="",A,H,R,P;async function ze(t){t.preventDefault();const e=document.querySelector(".modal-get-raiting").dataset.id,a=document.querySelector(".rating-container-js").dataset.rating,o=document.querySelector(".raiting-form-field-input").value,n=document.querySelector(".raiting-form-field-comment").value,i={rate:Number(a),email:o,review:n},l=await c.addExercisesRate(e,i);l.message?f.error(`${l.message}`):(f.success(`Thank you for your mark - ${i.rate} for ${l.name}`),u.close())}async function Qe(){const t=document.querySelector(".modal-info").dataset.id;u.close(),u.open(xe(t)),Fe(),P=document.querySelector(".raiting-form"),P.addEventListener("submit",ze)}async function Ke(t){const e=document.querySelector(".refresh-button-js"),s=document.querySelector(".modal-info").dataset.id;if(e.dataset.favorite==="false"){e.innerHTML=`<button class="add-favorite-js" type="button">
                                        <span class="remote-favorites">Remove from favorites</span>
                                        <svg class="trash-icon-img" width="18" height="18" aria-label="trash-icon">
                                            <use href="${Y}#icon-trash"></use>
                                        </svg>
                                    </button>`,e.dataset.favorite="true";let a=await c.getOneExercises(s);Ue(a)}else e.innerHTML=`<button class="add-favorite-js" type="button">
                                        <span>Add to favorites</span>
                                        <svg class="heart-icon-img" width="20" height="20" aria-label="heart-icon">
                                            <use href="./img/svg/sprite.svg#icon-heart"></use>
                                        </svg>
                                    </button>`,e.dataset.favorite="false",getRatingButton.style.fontSize="16px",De(s)}function Ge({target:t}){if(t.nodeName==="BUTTON"){if(t.nodeName==="BUTTON")return O=t.dataset.id,A=Ye(O);ifStartButton(t)&&u.open($(A))}}Je.addEventListener("click",Ge);const Ye=async t=>{let e=await c.getOneExercises(t);const s=localStorage.getItem("favorites");console.log(s),s!==null&&JSON.parse(s).find(({_id:n})=>n===t)?e.favorite=!0:e.favorite=!1,u.open($(e)),R=document.querySelector(".refresh-button-js"),H=document.querySelector(".add-rating"),H.addEventListener("click",Qe),R.addEventListener("click",Ke)},W=document.createElement("div");W.id="spinner-container";document.body.appendChild(W);const We={lines:13,length:20,width:3,radius:30,scale:1,corners:1,speed:1,rotate:0,animation:"spinner-line-fade-quick",direction:1,color:"#f4f4f4",fadeColor:"transparent",top:"50%",left:"50%",shadow:"0 0 1px transparent",zIndex:2e9,className:"spinner",position:"absolute"};document.getElementById("spinner-container");new ie(We);
//# sourceMappingURL=spinner-f6a62c66.js.map
