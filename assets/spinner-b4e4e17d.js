var O=Object.defineProperty;var $=(t,e,s)=>e in t?O(t,e,{enumerable:!0,configurable:!0,writable:!0,value:s}):t[e]=s;var w=(t,e,s)=>($(t,typeof e!="symbol"?e+"":e,s),s);import{g as R,a as F,i as U,b as j,G as h,s as J,c as K,S as D}from"./vendor-938032cf.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&o(r)}).observe(document,{childList:!0,subtree:!0});function s(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerpolicy&&(i.referrerPolicy=n.referrerpolicy),n.crossorigin==="use-credentials"?i.credentials="include":n.crossorigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(n){if(n.ep)return;n.ep=!0;const i=s(n);fetch(n.href,i)}})();const E=document.getElementById("home"),x=document.getElementById("favorites"),S=document.getElementById("home-text"),_=document.getElementById("favorites-text");document.getElementById("navigation");const z=window.location.href.toString();let k=z.slice(-10);const G=()=>{console.log(k),k==="index.html"?(E.classList.add("active"),S.classList.add("black"),x.classList.remove("active"),_.classList.remove("black")):(E.classList.remove("active"),S.classList.remove("black"),x.classList.add("active"),_.classList.add("black"))};G();const Y=document.getElementById("burger-button"),b=document.getElementById("burger"),Q=document.getElementById("burger-button-close"),W=async()=>{b.classList.remove("visually-hidden"),document.body.classList.add("disable-scroll")},V=async t=>{b.classList.add("visually-hidden"),document.body.classList.remove("disable-scroll")},X=t=>{t.key==="Escape"&&b.classList.add("visually-hidden"),document.body.classList.remove("disable-scroll")};Y.addEventListener("click",W);Q.addEventListener("click",V);document.addEventListener("keydown",X);const Z={apiKey:"AIzaSyBNBMjRxNAY7Ev3P0n12UWF7CdjphLNQ5M",authDomain:"hosting-test-2fa75.firebaseapp.com",projectId:"hosting-test-2fa75",storageBucket:"hosting-test-2fa75.appspot.com",messagingSenderId:"882269537085",appId:"1:882269537085:web:b0e1d25d79d88d76b4bb54"},ee=R().length?F():U(Z),I=j(ee),te=new h,m=document.getElementById("sign-in"),g=document.getElementById("sign-out"),L=document.getElementById("user");L.innerHTML=localStorage.getItem("user");localStorage.getItem("user")&&(g.classList.remove("display-none"),m.classList.add("display-none"));const se=async()=>{await J(I,te).then(t=>{h.credentialFromResult(t).accessToken;const s=t.user;localStorage.user=s.email,L.innerHTML=s.email,g.classList.remove("display-none"),m.classList.add("display-none")}).catch(t=>{t.message,t.customData.email;const e=h.credentialFromError(t);console.log(e)})},ne=async()=>{await K(I).then(()=>{L.innerHTML="",localStorage.removeItem("user"),console.log("Sign-out successful.")}).catch(t=>{console.log(t.message)}),g.classList.add("display-none"),m.classList.remove("display-none")};m.addEventListener("click",se);g.addEventListener("click",ne);const oe=document.querySelector(".modal-js");oe.addEventListener("click",ce);class ae{constructor(){this.overlay=document.querySelector(".overlay"),this.modal=document.querySelector(".modal-info"),this.closeButtonHandler=()=>this.close(),this.escapeKeyHandler=e=>this.handleEscapeKey(e),this.overlayClickHandler=e=>this.handleOverlayClick(e)}open(e){this.overlay.innerHTML=e,this.overlay.style.display="flex",this.modal.classList.remove("visually-hidden"),document.querySelector(".modal-button-close").addEventListener("click",this.closeButtonHandler),document.addEventListener("keydown",this.escapeKeyHandler),this.overlay.addEventListener("click",this.overlayClickHandler)}close(){this.modal=document.querySelector(".modal-info")||document.querySelector(".modal-get-raiting"),this.overlay.style.display="none",this.modal.classList.add("visually-hidden"),this.closeButton.removeEventListener("click",this.closeButtonHandler),document.removeEventListener("keydown",this.escapeKeyHandler),this.overlay.removeEventListener("click",this.overlayClickHandler)}handleEscapeKey(e){e.key==="Escape"&&this.close()}handleOverlayClick(e){e.target===this.overlay&&this.close()}}const ie=new ae,re=`<div class="modal-get-raiting">
  ...
  </div>`;function ce(t){ie.open(re)}const a=class{static handleErrors(e){return async()=>{try{return await e()}catch(s){console.error("Помилка при запиті: ",s.message)}}}static async fetchJson(e,s){return(await fetch(e,s)).json()}async getQuotes(){return await a.handleErrors(async()=>await a.fetchJson(`${a.BASE_URL}/quote`))()}async getByFilterName(e){return await a.handleErrors(async()=>{const o=new URLSearchParams({...e});return await a.fetchJson(`${a.BASE_URL}/filters?${o}`)})()}async getByFilterCategory(e){return await a.handleErrors(async()=>{const o=new URLSearchParams({...e});return await a.fetchJson(`${a.BASE_URL}/exercises?${o}`)})()}async getOneExercises(e){return await a.handleErrors(async()=>await a.fetchJson(`${a.BASE_URL}/exercises/${e}`))()}async addExercisesRate(e,s){return await a.handleErrors(async()=>{const n={method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)};return await a.fetchJson(`${a.BASE_URL}/exercises/${e}/rating`,n)})()}async addSubscription(e){return await a.handleErrors(async()=>{const o={method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)};return await a.fetchJson(`${a.BASE_URL}/subscription`,o)})()}};let d=a;w(d,"BASE_URL","https://your-energy.b.goit.study/api");const p=new d,y=document.querySelector(".pagination-numbers"),le=document.querySelector(".muscles-list"),f=document.getElementById("next-button"),v=document.getElementById("prev-button");let de=5,u=1;try{y.addEventListener("click",ue),y.addEventListener("click",M)}catch(t){console.log(t)}async function ue(t){const{totalPages:e,categoryName:s}=JSON.parse(localStorage.getItem("infoRequest")),o={filter:s,page:t.target.textContent,limit:12,totalPages:e},i=(await p.getByFilterName(o)).results;le.innerHTML=H(i),u=o.page,pe()}const me=t=>{const e=document.createElement("button");e.className="pagination-number",e.innerHTML=t,e.setAttribute("page-index",t),e.setAttribute("aria-label","Page "+t),y.appendChild(e)};function ge(){const{totalPages:t}=JSON.parse(localStorage.getItem("infoRequest"));if(t!==1)for(let e=1;e<=t;e++)me(e)}function pe(t){c(),v.addEventListener("click",()=>{c()}),f.addEventListener("click",()=>{c()}),document.querySelectorAll(".pagination-number").forEach(e=>{Number(e.getAttribute("page-index"))&&e.addEventListener("click",()=>{c()})})}function c(t){M(),he()}function M(){document.querySelectorAll(".pagination-number").forEach(t=>{t.classList.remove("active"),Number(t.getAttribute("page-index"))==u&&t.classList.add("active")})}const B=t=>{t.classList.add("disabled"),t.setAttribute("disabled",!0)},N=t=>{t.classList.remove("disabled"),t.removeAttribute("disabled")},he=()=>{u===1?B(v):N(v),de===u?B(f):N(f)},ye=document.querySelector(".wrap-button"),A=document.querySelector(".muscles-list"),fe=document.querySelector(".pagination-numbers");ye.addEventListener("click",ve);document.addEventListener("DOMContentLoaded",()=>{q({filter:"Muscles",page:1,limit:12})});async function ve(t){t.preventDefault();const{target:e,currentTarget:s}=t;if(e.nodeName!=="BUTTON")return;const o=e.dataset.name;e.classList.contains("btn-filter")&&[...s.children].forEach(r=>{r.firstElementChild.classList.remove("btn-filter-active")}),e.classList.add("btn-filter-active");const n={filter:o,page:1,limit:12};A.innerHTML="",q(n)}async function q(t){console.log(t);try{const e=await p.getByFilterName(t),s=e.results;if(!e||s.length===0){console.log("Sorry, we didn't find anything according to your request.");return}A.insertAdjacentHTML("beforeend",H(s));const{totalPages:o}=e,n=JSON.stringify({totalPages:o,categoryName:t.filter});localStorage.setItem("infoRequest",n),fe.innerHTML="",ge(),c(1)}catch(e){console.log(e.message)}}function H(t){return t.map(({filter:s,name:o,imgURL:n})=>`
        <li class="muscles-item"  data-name=${o} data-filter=${s.toLocaleLowerCase().replaceAll(" ","")}>
...
//# sourceMappingURL=spinner-b4e4e17d.js.map
