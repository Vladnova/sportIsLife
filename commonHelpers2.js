...
<p class="exercises_list_item_bottom_list_item_text"">Target: <span>${L(i,7)}</span></p>
    ...
    </li>`).join("");m.innerHTML=e}}const k=document.querySelector(".form-js"),z=document.querySelector(".pagination-numbers");k.addEventListener("submit",G);async function G(t){t.preventDefault();const e=t.target.elements.search.value.trim();if(!e)return;const{categoryName:s}=JSON.parse(localStorage.getItem("infoRequest")),r={[s.toLocaleLowerCase().replaceAll(" ","")]:e,page:1,limit:10};try{const a=await c.getByFilterCategory(r);a.results.length||o.info("Nothing was found for this query"),z.innerHTML="";const{totalPages:n}=a;h(n,r),u(1),k.reset(),x(a)}catch(a){console.log(a)}}const T=document.querySelector("#subscribe-form"),p=T.querySelector("#email");T.addEventListener("submit",J);async function J(t){t.preventDefault();const{value:e}=p;if(!/^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/.test(e)){o.info("Enter the following sample email - 'test@gmail.com'"),p.value="";return}try{const i=await c.addSubscription({email:e});o.success(i.message)}catch(i){console.log(i)}finally{p.value=""}}
