...
<p class="exercises_list_item_bottom_list_item_text"">Target: <span>${L(i,7)}</span></p>
    ...
    </li>`).join("");m.innerHTML=e}}const k=document.querySelector(".form-js"),J=document.querySelector(".pagination-numbers");k.addEventListener("submit",U);async function U(t){t.preventDefault();const e=t.target.elements.search.value.trim();if(!e)return;const{categoryName:s}=JSON.parse(localStorage.getItem("infoRequest")),r={[s.toLocaleLowerCase().replaceAll(" ","")]:e,page:1,limit:10};try{const a=await c.getByFilterCategory(r);J.innerHTML="";const{totalPages:n}=a;h(n,r),u(1),k.reset(),x(a)}catch{}}const w=document.querySelector("#subscribe-form"),f=w.querySelector("#email");w.addEventListener("submit",Z);async function Z(t){t.preventDefault();const{value:e}=f;if(!/^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/.test(e)){o.info("Enter the following sample email - 'test@gmail.com'"),f.value="";return}try{const i=await c.addSubscription({email:e});o.success(i.message)}catch(i){console.log(i)}finally{f.value=""}}const C=document.createElement("div");C.id="spinner-container";document.body.appendChild(C);const K={lines:13,length:20,width:3,radius:30,scale:1,corners:1,speed:1,rotate:0,animation:"spinner-line-fade-quick",direction:1,color:"#f4f4f4",fadeColor:"transparent",top:"50%",left:"50%",shadow:"0 0 1px transparent",zIndex:2e9,className:"spinner",position:"absolute"};document.getElementById("spinner-container");new P(K);
